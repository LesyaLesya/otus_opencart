...
ВАЖНО:
в Jenkinsfile указан полный путь до исполняемого файла docker: /usr/local/bin/docker .
Если путь отличается, то нужно исправить его в Jenkinsfile и запушить изменения в собственный репозиторий.
Его же тогда указывать в настройках пайплайна.
____
